...
- 支持通过 GUI 开关控制消息展示项
- 仅在状态变化时推送通知，减少刷屏
- **推送目标支持 UMO**（AI 修改版）：`/mc_push_target <UMO|QQ群号|本群>` 聊天内动态设置推送群，无需改 WebUI
...
- 推送目标支持 UMO（`/mc_push_target <UMO|QQ群号|本群>`），可动态切换推送群。
...
- `/mc_start_server_monitor` 启动监控任务
- `/mc_stop_server_monitor` 停止监控任务
- `/mc_query` 立即查询当前所有已启用服务器的状态
- `/mc_reset_monitor` 重置监控缓存（管理员）
- `/mc_push_target` 查看当前推送目标与用法（管理员）
- `/mc_push_target <UMO>` 按 UMO 设置推送目标，如 `/mc_push_target atri:GroupMessage:1092815819`，并发送测试消息（管理员）
- `/mc_push_target <QQ群号>` 兼容旧版：按纯数字群号设置（管理员）
- `/mc_push_target 本群` 把当前会话设为推送目标（管理员）
- `/mc_push_target 清除` 清除聊天下令设置，恢复 WebUI 面板的 `target_umo` / `target_group`（管理员）
- `/mc_push_test` 向当前生效目标发送测试消息（管理员）
